class NoticiasController < ApplicationController
  ...
  # GET /noticias.json
  def index
    @noticias = Noticia.all.order('id DESC').page(params[:page]).per_page(6).search(params[:search])
  end
  ...
  private
    ...
    def set_noticia
      @noticia = Noticia.find_by_slug(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def noticia_params
      params.require(:noticia).permit(:slug, :titulo, :foto_portada, :contenido, :estado)
    end
end
